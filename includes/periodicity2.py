...
class AppForm(QDialog):
    ...
    def on_draw(self):
        ...
        txt_data = map(int, str.split())
        
        data_fft = np.fft.rfft(data_1)
        ...
        if len(txt_data) == 2:
            ...
            for i in range(col_cnt):
                ...
                temp[-a:-b] = 0
                global out_array
                out_array[:,i] = np.fft.irfft(temp, n=len(data[:,i]), axis=0)
...
def periodic_filter(data):
    #split data into number of columns
#    col_cnt = data.shape[1]
    global data_1
    data_1 = data[:,0]
